	...
	private function retrieve_theme_update_details( $theme ) {
		...
		if ( ( $response_code = intval( wp_remote_retrieve_response_code( $response ) ) ) !== 200 ) {
			FW_Flash_Messages::add(
				...
				esc_html__( 'An error occurred, please try again later. ', 'start' ),
				'error'
			);
		...
		{ //Notification of the need to add a license key to receive updates.
			...
			if ( empty( $update_licence_key ) ) {
				$notices['need_licence_key'] = array(
					...
					'data'      => array(
						'message' => '<strong>' . sprintf( esc_html__( 'It needs to fill Licence key field from <a href="%s">Theme Settings</a> page, that you received by email so you can receive updates.', 'start' ), admin_url( 'themes.php?page=fw-settings' ) ) . '</strong>',
						'type'    => 'warning'
					)
				);
